...
final class YamlVisitor<N extends ConfigurationNode> implements ConfigurationVisitor<N, YamlVisitor.State, Void, IOException> {
    ...
    @Override
    public void beginVisit(final N node, final State state) throws IOException {
        state.start = node;
    ...
    @Override
    public void enterNode(final N node, final State state) throws IOException {
        if (node != state.start && node.getKey() != null && node.getParent().isMap()) { // emit key
            final String value = String.valueOf(node.getKey());
    ...
    @Override
    public void enterMappingNode(final N node, final State state) throws IOException {
    ...
    @Override
    public void enterListNode(final N node, final State state) throws IOException {
    ...
    @Override
    public void enterScalarNode(final N node, final State state) throws IOException {
        ...
        state.emit(new ScalarEvent(anchor(node), implicit.getValue(), implicity, value, null, null,
                ScalarStyle.asSnakeYaml(node.getHint(YamlConfigurationLoader.SCALAR_STYLE))));
    }

    // TODO: emit alias events for enterReferenceNode

    @Override
    public void exitMappingNode(final N node, final State state) throws IOException {
        state.emit(MAPPING_END);
    }

    @Override
    public void exitListNode(final N node, final State state) throws IOException {
    ...
    private @Nullable NodeStyle determineStyle(final N node) {
        return node.getHint(YamlConfigurationLoader.NODE_STYLE);
    }

    private @Nullable String anchor(final N node) {
        return node.getOwnHint(YamlConfigurationLoader.ANCHOR_ID);
    }
